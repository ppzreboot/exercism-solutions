...
updateGoalCountForPlayer playerName playerGoalCounts =
    Dict.update
        playerName
        (\old -> Just (1 + (Maybe.withDefault 0 old)))
        playerGoalCounts
...
formatPlayer playerName playerGoalCounts =
    let
        maybeScore = Dict.get playerName playerGoalCounts
        score = Maybe.withDefault 0 maybeScore
    in
        formatPPz (playerName, score)
...
addScorePPz (name, scoreIncr) dict =
    let
        oldScore = Dict.get name dict
        newScore = Maybe.withDefault scoreIncr oldScore
    in
        Dict.update name (\_ -> Just newScore) dict

combineGames: Dict PlayerName Int -> Dict PlayerName Int -> Dict PlayerName Int
combineGames game1 game2 =
    let
        addOne key newValue old =
            Dict.update key (\_ -> Just newValue) old
        addTwo key valueL valueR old =
            Dict.update key (\_ -> Just (valueL + valueR)) old
    in
        Dict.merge
            addOne
            addTwo
